...
- Next.js (App Router) + React + TypeScript.
  ...
  - Apify Actors API (3 actors Instagram IG-only — Vault não usa dataset de comentários desde 2026-09-01).
  - Vitest + ESLint.
...
4. **Pool global de tokens Apify:** qualquer token ativo **com credito** (estimativa `1000 − uso`) serve. Ate `SCRAPE_MAX_PARALLEL_KEYS` (**100**) workers em paralelo; cada chave processa 1 perfil por vez. **IG-only:** não-Instagram é pulado com `unsupported_platform` (aviso, sem custo).
5. Instagram: 3 actors em paralelo (profile + post `resultsLimit: 5` + reel `resultsLimit: 5`). TikTok: pausado (pulado sem custo).
...
- **Tudo em Supabase + Render — nada local (regra 2026-08-30, rígida desde 2026-09-07).** `DATABASE_URL` (pooler `6543`) e `DIRECT_URL` (migrations `5432`) são `postgresql://` do Supabase em **todos** os ambientes (`next dev` local e `scripts/start.mjs` em prod). `prisma/dev.db` foi **deletado**; fallbacks `file:`/SQLite removidos de `src/lib/scrape-reconcile.ts`, `history/page.tsx` e `api/history/route.ts`. Sem `.env` Supabase, o app falha alto em `assertSupabaseDatabaseUrl()` / `requireDatabaseEnvironment()` / `prisma validate` — sem fallback silencioso.
- PostgreSQL remoto no Supabase; chaves ficam armazenadas no banco remoto e nunca entram no Git.
- `estimatedCredits` = `recordsReceived` (registros entregues — proxy operacional). Fatura = painel Apify.
