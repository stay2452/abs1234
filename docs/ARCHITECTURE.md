...
6. Persistencia por perfil em transacao: upsert de posts por `[profileId, url, sourceType]`; biblioteca **acumula**; metricas iguais nao geram novo `PostSnapshot`.
...
| `Post` | Conteudo; identidade `[profileId, url, sourceType]`; `platform` denormalizado de `Profile` |
...
- SQLite no PC do usuario; chaves so no SQLite (gitignored).
- `estimatedCredits` ≈ `recordsReceived` (proxy). Fatura = painel Bright Data.
