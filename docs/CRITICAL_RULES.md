...
- **Proteção opt-in dos endpoints que gastam crédito (2026-09-01, endurecida em 2026-09-08):** com contas de usuário ativas, o legado "aberto sem token" acabou — `hasValidApiToken()` (`src/lib/auth.ts`) só autoriza com token explícito; sem token, vale a sessão de login (`apiGuard`/`isApiAllowed`). `POST /api/scrape/run`, `POST /api/metrics/repair` e `GET /api/scrape/estimate` aceitam **qualquer logado (só os próprios perfis)** ou token da extensão/operador (onde aplicável: run, repair, estimate, import); `scrape/session`, Discord e auditoria/history seguem admin-only. Leitura (rankings, pastas, vault, creators) exige login; escrita em vault/creators/pastas exige o próprio dono (admin passa). A extensão precisa de `API_ACCESS_TOKEN` configurado nos dois lados (app + `chrome.storage.sync.apiToken`) — sem ele, as chamadas dela sem sessão caem em 401. `CRON_SECRET` protege `/api/cron/reconcile` da mesma forma (recomendado definir no Render).
...
- **Biblioteca pessoal por usuário (2026-09-08):** `Profile`/`Folder`/`Creator` têm `ownerId` (FK `User`, `onDelete: Cascade`); unicidade de perfil é `[ownerId, platform, handle]` — mesmo `@handle` pode existir em contas diferentes. Usuário comum só vê/cadastra/atualiza/apaga o próprio (`ownerWhere`/`canAccessOwner` em `src/lib/ownership.ts`); inexistente ou alheio responde 404 igual (não vaza existência). Coleta, estimate e repair filtram por dono (`runScrape({ownerId})`); quem disparou vai em `ScrapeRun.triggeredById`. Chaves Apify continuam em **pool global** gerido pelo admin; auditoria/history, chaves e Discord seguem admin-only. Admin enxerga todas as bibliotecas (moderação). Backfill: linhas pré-existentes foram para a conta mais antiga.
- **Tokens pessoais da extensao — modo SaaS (2026-09-11):** cada usuario loga a extensao com email+senha (`POST /api/auth/extension-token`, com o mesmo rate-limit do login) e recebe um token proprio `eoz_...` (model `ApiToken`: hash scrypt, prefixo indexado, revogavel; texto exibido uma unica vez). Import/coleta via Bearer pessoal cai na biblioteca **do dono** (`getRequestUser` resolve o token antes do fallback de admin mais antigo, que segue valendo so para o `API_ACCESS_TOKEN` global do operador). Gestao: `GET/POST /api/users/api-tokens`, `DELETE /api/users/api-tokens/[id]` (sessao, so os proprios) e `DELETE /api/auth/extension-token` (logout da extensao revoga o proprio Bearer). Fonte: `src/lib/api-tokens.ts`.
